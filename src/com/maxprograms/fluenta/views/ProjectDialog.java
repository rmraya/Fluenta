	...
	public ProjectDialog(Shell parent, int style, Project proj) {
		...
		mapBrowse.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent event) {
				...
				fd.setFilterNames(new String[]{Messages.getString("ProjectDialog.8"), Messages.getString("ProjectDialog.9"), Messages.getString("ProjectDialog.10")}); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
		...
		if (project.getSrcLanguage() != null) {
			sourceLanguages.select(TextUtils.geIndex(sourceLanguages.getItems(), LanguageUtils.getLanguageName(project.getSrcLanguage().getCode())));
		} else {
			try {
		...
		create.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent event) {
				...
				if (project.getId() == 0l) {
					...
					Project p = new Project(id, titleText.getText(), descriptionText.getText(), System.getProperty("user.name"), mapText.getText(), new Date(),  //$NON-NLS-1$
							Project.NEW, null, srcLang,  targetLangs, new Vector<Memory>());
					Memory m = new Memory(id, p.getTitle(), p.getDescription(), p.getOwner(), new Date(), null, p.getSrcLanguage(), new Vector<Language>());
					p.getMemories().add(m);					
					try {
