...
public class ProjectsView extends Composite {
	...
	public ProjectsView(Composite parent, int style) {
		...
		Listener sortListener = new Listener() {
			...
			public void handleEvent(Event e) {
				...
				if (column == description) sortField = 0;
				if (column == map) sortField = 1;
				if (column == status) sortField = 2;
				if (column == created) sortField = 3;
				if (column == updated) sortField = 4;
				loadProjects();
			}
		};
	...
	public void loadProjects() {
		...
		try {
			...
			final Collator collator = Collator.getInstance(new Locale(Fluenta.getLanguage()));
			Arrays.sort(array, new Comparator<Project>() {
				...
				public int compare(Project o1, Project o2) {
					if (table.getSortDirection() == SWT.UP) {
						switch (sortField) {
						case 0:
							return collator.compare(o1.getTitle().toLowerCase(Locale.getDefault()), o2.getTitle().toLowerCase(Locale.getDefault()));
						case 1:
							return collator.compare(o1.getMap().toLowerCase(Locale.getDefault()), o2.getMap().toLowerCase(Locale.getDefault()));
						case 2:
					...
					switch (sortField) {
					case 0:
						return collator.compare(o2.getTitle().toLowerCase(Locale.getDefault()), o1.getTitle().toLowerCase(Locale.getDefault()));
					case 1:
						return collator.compare(o2.getMap().toLowerCase(Locale.getDefault()), o1.getMap().toLowerCase(Locale.getDefault()));
					case 2:
			...
			for (int i=0 ; i<array.length ; i++) {
				...
				item.setText(new String[]{p.getTitle(), p.getMap(), p.getStatus(), p.getCreationDateString(), p.getLastUpdateString()});
			}
		} catch (IOException e) {
			e.printStackTrace();
	...
	public void updateProject() {
		...
		ProjectDialog updateProject = new ProjectDialog(getShell(), SWT.DIALOG_TRIM|SWT.RESIZE, (Project) table.getSelection()[0].getData("project")); //$NON-NLS-1$
		updateProject.show();
	}
	...
	public void projectDetails() {
		...
		ProjectInfoDialog dialog = new ProjectInfoDialog(getShell(), SWT.DIALOG_TRIM|SWT.RESIZE, (Project) table.getSelection()[0].getData("project")); //$NON-NLS-1$
		dialog.show();
	}
	...
	public void generateXliff() {
		...
		GenerateXliffDialog dialog = new GenerateXliffDialog(getShell(), SWT.DIALOG_TRIM, (Project)table.getSelection()[0].getData("project") ); //$NON-NLS-1$
		dialog.show();
	}
	...
	public void importXliff() {
		...
		ImportXliffDialog dialog = new ImportXliffDialog(getShell(), SWT.DIALOG_TRIM, (Project)table.getSelection()[0].getData("project")); //$NON-NLS-1$
		dialog.show();
	}
