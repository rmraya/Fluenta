...
public class ProjectInfoDialog extends Dialog {
	...
	public ProjectInfoDialog(Shell parent, int style, Project project) {
		...
		statusTable = new Table(statusComposite, SWT.V_SCROLL|SWT.H_SCROLL|SWT.FULL_SELECTION|SWT.SINGLE|SWT.CHECK|SWT.BORDER);
		...
		Table eventsTable = new Table(historyComposite, SWT.V_SCROLL|SWT.SINGLE|SWT.H_SCROLL|SWT.FULL_SELECTION|SWT.BORDER);
		...
		while (it.hasNext()) {
			...
			item.setText(new String[]{event.getDateString(), "" + event.getBuild(), LanguageUtils.getLanguageName(event.getLanguage().getCode()), ProjectEvent.getDescription(event.getType())}); //$NON-NLS-1$
		}
	...
	protected void populateStatusTable(Project project) {
		...
		while (tl.hasNext()) {
			...
			item.setText(new String[]{LanguageUtils.getLanguageName(l.getCode()), project.getTargetStatus(l.getCode())});
