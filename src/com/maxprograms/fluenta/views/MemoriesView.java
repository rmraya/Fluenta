...
public class MemoriesView extends Composite {
	...
	public MemoriesView(Composite parent, int style) {
		...
		Listener sortListener = new Listener() {
			...
			public void handleEvent(Event e) {
				...
				if (column == created) sortField = 1;
				if (column == updated) sortField = 2;
				loadMemories();
			}
		};
	...
	public void loadMemories() {
		...
		try {
			...
			Arrays.sort(array, new Comparator<Memory>() {
				...
				public int compare(Memory o1, Memory o2) {
					if (table.getSortDirection() == SWT.UP) {
						switch (sortField) {
						case 0:
							return collator.compare(o1.getName().toLowerCase(Locale.getDefault()), o2.getName().toLowerCase(Locale.getDefault()));
						case 1:
					...
					switch (sortField) {
					case 0:
						return collator.compare(o2.getName().toLowerCase(Locale.getDefault()), o1.getName().toLowerCase(Locale.getDefault()));
					case 1:
			...
			for (int i=0 ; i<array.length ; i++) {
				...
				item.setText(new String[]{m.getName(), m.getCreationDateString().toString(), m.getLastUpdateString()});
			}
		} catch (IOException e) {
			e.printStackTrace();
	...
	private void exportTMX(Memory memory) {
		...
		fd.setFilterNames(new String[]{Messages.getString("MemoriesView.22"), Messages.getString("MemoriesView.23")}); //$NON-NLS-1$ //$NON-NLS-2$
