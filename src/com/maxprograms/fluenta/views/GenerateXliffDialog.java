	...
	public GenerateXliffDialog(Shell parent, int style,  Project project) {
		...
		ditavalBrowse.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent event) {
				...
				fd.setFilterNames(new String[]{Messages.getString("GenerateXliffDialog.8"), Messages.getString("GenerateXliffDialog.9")}); //$NON-NLS-1$ //$NON-NLS-2$
		...
		for (int i=0 ; i<languages.size() ; i++) {
			...
			targets[i].setText(LanguageUtils.getLanguageName(languages.get(i).getCode()));
		...
		generateXliff.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent event) {
				...
				thread = new Thread() {
					...
					@Override
					public void run() {
						MainView.getController().generateXliff(project, xliffFolder, tgtLangs, useice, usetm , count, ditaval, useXliff20, alogger);
					}
